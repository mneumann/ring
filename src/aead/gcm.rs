...
impl Context {
    pub(crate) fn new(key: &Key, aad: Aad<&[u8]>, cpu_features: cpu::Features) -> Self {
        ...
        for ad in aad.0.chunks(BLOCK_LEN) {
            ...
            (&mut padded[..ad.len()]).copy_from_slice(ad);
    ...
    where
        F: FnOnce([u8; BLOCK_LEN]) -> super::Tag,
    {
        let xi = self.inner.Xi.0;
        let xi: [BigEndian<u64>; 2] = [xi[0].into(), xi[1].into()];
        f(*xi.as_byte_array())
    }
...
#[inline]
pub fn bytes_to_u64s(a: [u8; BLOCK_LEN]) -> [BigEndian<u64>; 2] {
    let a0: &[u8; BLOCK_LEN / 2] = (&a[..(BLOCK_LEN / 2)]).try_into().unwrap();
    let a1: &[u8; BLOCK_LEN / 2] = (&a[(BLOCK_LEN / 2)..]).try_into().unwrap();
    [(*a0).into(), (*a1).into()]
}
